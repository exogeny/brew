...
HOMEBREW_DEFAULT_PREFIX="${HOME}/.local/brew"
HOMEBREW_DEFAULT_REPOSITORY="${HOMEBREW_DEFAULT_PREFIX}"
if [[ -n "${HOMEBREW_MACOS}" ]]
then
  ...
  HOMEBREW_DEFAULT_CACHE="${CACHE_HOME}/brew"
  HOMEBREW_DEFAULT_LOGS="${CACHE_HOME}/brew/logs"
...
case "$1" in
  formulae)
    source "${HOMEBREW_LIBRARY}/cmd/formulae.sh"
    brew-formulae
    exit 0
    ;;
  casks)
    source "${HOMEBREW_LIBRARY}/cmd/casks.sh"
    brew-casks
    exit 0
    ;;
  shellenv)
    source "${HOMEBREW_LIBRARY}/cmd/shellenv.sh"
    shift
    brew-shellenv "$1"
    exit 0
    ;;
  setup-ruby)
    source "${HOMEBREW_LIBRARY}/cmd/setup-ruby.sh"
    shift
    brew-setup-ruby "$1"
    exit 0
    ;;
...
source "${HOMEBREW_LIBRARY}/help.sh"
...
case "$@" in
  ...
  --prefix* | --cellar*)
    source "${HOMEBREW_LIBRARY}/formula_path.sh"
    brew-formula-path "$@" && exit 0
    ;;
  # falls back to cmd/command.rb on a non-zero return
  command*)
    source "${HOMEBREW_LIBRARY}/command_path.sh"
    brew-command-path "$@" && exit 0
    ;;
  # falls back to cmd/list.rb on a non-zero return
  list* | ls*)
    source "${HOMEBREW_LIBRARY}/list.sh"
    brew-list "$@" && exit 0
    ;;
  # brew-tap only handles invocations with no arguments
  tap)
    source "${HOMEBREW_LIBRARY}/tap.sh"
    brew-tap "$@"
    exit 0
    ;;
...
if [[ -z "${HOMEBREW_VERSION}" ]]
then
  HOMEBREW_VERSION=">=1.0.0 (shallow or no git repository)"
  HOMEBREW_USER_AGENT_VERSION="1.X.Y"
fi

HOMEBREW_CORE_REPOSITORY="${HOMEBREW_LIBRARY}/taps/homebrew-core"
# Used in --version.sh
# shellcheck disable=SC2034
HOMEBREW_CASK_REPOSITORY="${HOMEBREW_LIBRARY}/taps/homebrew-cask"
...
case "$1" in
  --version | -v)
    source "${HOMEBREW_LIBRARY}/cmd/--version.sh"
    brew-version
    exit 0
    ;;
esac
...
if [[ -f "${HOMEBREW_LIBRARY}/dev-cmd/${HOMEBREW_COMMAND}.sh" ]] ||
   [[ -f "${HOMEBREW_LIBRUBY}/dev-cmd/${HOMEBREW_COMMAND}.rb" ]]
then
...
if [[ -f "${HOMEBREW_LIBRARY}/cmd/${HOMEBREW_COMMAND}.sh" ]]
then
  HOMEBREW_BASH_COMMAND="${HOMEBREW_LIBRARY}/cmd/${HOMEBREW_COMMAND}.sh"
elif [[ -f "${HOMEBREW_LIBRARY}/dev-cmd/${HOMEBREW_COMMAND}.sh" ]]
then
  HOMEBREW_BASH_COMMAND="${HOMEBREW_LIBRARY}/dev-cmd/${HOMEBREW_COMMAND}.sh"
fi
...
source "${HOMEBREW_LIBRARY}/utils/analytics.sh"
setup-analytics
